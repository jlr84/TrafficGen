...
class BenchmarkTopo( Topo ):
    ...
    def __init__( self, **opts ):
        ...
        for i in range(1,41):
            ...
            self.addLink( 'h'+str(i), sw1, bw=1000, delay='5ms', port1=i)

        # Add receiver hosts
        receivers = set()
        for i in range(201,241):
            receivers.add(self.addHost('h'+str(i),ip='10.0.0.'+str(i-120)+'/8'))
            self.addLink( 'h'+str(i), sw2, bw=1000, delay='5ms', port1=i-200)
...
#        h500 = self.addHost( 'log', ip='10.0.0.21/8' )
        ...
        #COUNTER=1
        #for i in senders:
        #    self.addLink( i, sw1, bw=1000, delay='5ms', port1=COUNTER)
        #    COUNTER=COUNTER+1

        # Add links to receiver hosts
        #COUNTER=1
        #for i in receivers:
        #    self.addLink( i, sw2, bw=1000, delay='5ms', port1=COUNTER)
        #    COUNTER=COUNTER+1

        # Add links 
        # Switches first
        self.addLink( sw1, sw2, bw=1000, delay='5ms', max_queue_size=1000, port1=313, port2=313 )
